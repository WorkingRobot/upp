    ...
    MapProperty::BaseProperty(FArchive& Ar, const UPropertyTag& Tag, EReadType ReadType)
    {
        ...
        case EReadType::Map:
            ...
            for (int i = 0; i < NumEntries; ++i) {
                Value.emplace_back(std::piecewise_construct, std::forward_as_tuple(Ar, *Tag.TagData.GetData().Map.InnerType, EReadType::Map), std::forward_as_tuple(Ar, *Tag.TagData.GetData().Map.ValueType, EReadType::Map));
            }
            break;
        }
    }
